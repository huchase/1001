<script>
import { blockMod, color, transition } from '../tetrisData'
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // 渲染形状数据
      this.renderBlock(this.nowBlock, this.frame, 1) // TODO:
      // 旋转
      if (this.now.c > 0) { // TODO:
...
<template>
  <div class="game">
    <div class="game-div">
      <div class="game-min">
        <div v-for="(row, i) in frame" :key="i" class="row">
          <p v-for="(col, j) in row" :key="j" class="element" :style="{ background: col.bg }" />
        </div>
      </div>
      <div class="right-div">
        <div class="ass">
    ...
    <div class="control">
      <div class="change bt" @click="change1()">
        变化
      </div>
      <div class="zy">
        <div class="left bt" @click="moveLeft()">
          向左
        </div>
        <div class="right bt" @click="moveRight()">
          向右
        </div>
      </div>
      <div class="down bt" @click="moveDown()">
        向下
      </div>
    </div>
  </div>
