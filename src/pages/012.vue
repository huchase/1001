...
`
  ...
  const cells = new Uint16Array(triangles) // TODO: WTF? cells...
  ...
  const colorLoc = gl.getUniformLocation(program, 'u_color') // TODO: uniform????????
  ...
  canvas.addEventListener('mousemove', (evt) => {
    ...
    const colorLoc = gl.getUniformLocation(program, 'u_color') // TODO: ????
    if (isPointInPath({ vertices, cells }, new Vector2D(offsetX, offsetY)))
      gl.uniform4fv(colorLoc, [0, 0.5, 0, 1]) // TODO:

    else
      gl.uniform4fv(colorLoc, [1, 0, 0, 1])

    gl.drawElements(gl.TRIANGLES, cells.length, gl.UNSIGNED_SHORT, 0) // TODO:
  })
